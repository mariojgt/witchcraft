<template>
    <div :class="`bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-xl p-5 min-w-[320px] relative shadow-2xl transition-all duration-300 node-${data.colorTheme || 'yellow'}`">
        <!-- Header -->
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3 flex-1 min-w-0">
                <div class="w-9 h-9 rounded-lg flex items-center justify-center border transition-all duration-300"
                    ...
                    <DownloadIcon class="w-5 h-5 transition-colors duration-300" :class="getIconColorClass()" />
                </div>
                <div class="flex-1 min-w-0">
                    <!-- Editable Title -->
                    <div v-if="!isEditingTitle"
                         @click="startEditingTitle"
                         class="font-semibold text-white text-sm cursor-pointer hover:text-yellow-400 transition-colors truncate"
                         :title="data.customTitle || 'Get Variable'">
                        {{ data.customTitle || 'Get Variable' }}
                    </div>
                    <input v-else
                           ...
                           ref="titleInput"
                           class="font-semibold text-white text-sm bg-transparent border-b border-yellow-500 outline-none w-full"
                           placeholder="Enter custom title" />

                    <div class="flex items-center gap-2 mt-0.5">
                        <p class="text-xs text-gray-400 leading-none truncate">
                            {{ data.customDescription || 'Retrieve stored data' }}
                        </p>
            ...
            <div class="flex items-center gap-1 ml-2">
                ...
                <button @click="toggleSettings"
                        class="w-8 h-8 rounded-lg hover:bg-yellow-500/10 text-gray-400 hover:text-yellow-400 transition-all duration-200 flex items-center justify-center group"
                        title="Node Settings">
        ...
        <div v-if="showSettings" class="mb-4 p-3 bg-white/5 border border-white/10 rounded-lg space-y-3">
            <div class="space-y-2">
                ...
                <input v-model="data.customTitle"
                       ...
                       class="w-full text-sm bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-gray-500 focus:bg-white/10 focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/20 transition-all duration-200 outline-none" />
            </div>
...
import { XIcon, DownloadIcon, CheckIcon, MessageSquareIcon, SettingsIcon } from 'lucide-vue-next'
...
function getHeaderIconClass() {
    const theme = props.data.colorTheme || 'yellow'
    const classes = {
    ...
    return classes[theme] || classes.yellow
}
...
function startEditingTitle() {
    ...
    editingTitle.value = props.data.customTitle || 'Get Variable'
    nextTick(() => {
