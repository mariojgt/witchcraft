        ...
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3 flex-1 min-w-0">
                <div class="w-9 h-9 rounded-lg flex items-center justify-center border transition-all duration-300"
                    ...
                    <BellIcon :class="`w-5 h-5 transition-colors duration-300 ${getIconColorClass()}`" />
                </div>
                <div class="flex-1 min-w-0">
                    ...
                    <div v-if="!isEditingTitle"
                         ...
                         :title="data.customTitle || 'Model Events'">
                        {{ data.customTitle || 'Model Events' }}
                    </div>
                    <input v-else
                    ...
                    <div class="flex items-center gap-2 mt-0.5">
                        <p class="text-xs text-gray-400 leading-none truncate">
                            {{ data.customDescription || 'Listen to database model events' }}
                        </p>
...
import { BellIcon, XIcon, CheckIcon, MessageSquareIcon, SettingsIcon } from 'lucide-vue-next';
...
function startEditingTitle() {
    ...
    editingTitle.value = props.data.customTitle || 'Model Events'
    nextTick(() => {
...
// Computed property for output placeholder
const getOutputPlaceholder = computed(() => {
    return props.data.eventType === 'created' ? 'New record data'
         : props.data.eventType === 'updated' ? 'Updated record data'
         : props.data.eventType === 'deleted' ? 'Deleted record data'
         : 'Event data'
})
